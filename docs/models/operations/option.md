...
```go
paraph.WithServerURL("https://api.example.com")
```
...
```go
paraph.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
paraph.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
paraph.WithTimeout(30 * time.Second)
```
