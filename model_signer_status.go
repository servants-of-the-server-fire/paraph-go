...
// SignerStatus Per-signer status. - `pending` — signing link sent, awaiting signer - `signed` — signer completed - `expired` — signer did not sign before `expires_at` - `error` — terminal failure for this signer - `cancelled` — parent request was cancelled 
type SignerStatus string

// List of SignerStatus
const (
	SIGNERSTATUS_PENDING SignerStatus = "pending"
	SIGNERSTATUS_SIGNED SignerStatus = "signed"
	SIGNERSTATUS_EXPIRED SignerStatus = "expired"
	SIGNERSTATUS_ERROR SignerStatus = "error"
	SIGNERSTATUS_CANCELLED SignerStatus = "cancelled"
)

// All allowed values of SignerStatus enum
var AllowedSignerStatusEnumValues = []SignerStatus{
	"pending",
	"signed",
	"expired",
	"error",
...
func (v *SignerStatus) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := SignerStatus(value)
	for _, existing := range AllowedSignerStatusEnumValues {
	...
	return fmt.Errorf("%+v is not a valid SignerStatus", value)
}

// NewSignerStatusFromValue returns a pointer to a valid SignerStatus
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewSignerStatusFromValue(v string) (*SignerStatus, error) {
	ev := SignerStatus(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for SignerStatus: valid values are %v", v, AllowedSignerStatusEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v SignerStatus) IsValid() bool {
	for _, existing := range AllowedSignerStatusEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to SignerStatus value
func (v SignerStatus) Ptr() *SignerStatus {
	return &v
}

type NullableSignerStatus struct {
	value *SignerStatus
	isSet bool
}

func (v NullableSignerStatus) Get() *SignerStatus {
	return v.value
}

func (v *NullableSignerStatus) Set(val *SignerStatus) {
	v.value = val
	v.isSet = true
}

func (v NullableSignerStatus) IsSet() bool {
	return v.isSet
}

func (v *NullableSignerStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSignerStatus(val *SignerStatus) *NullableSignerStatus {
	return &NullableSignerStatus{value: val, isSet: true}
}

func (v NullableSignerStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSignerStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
